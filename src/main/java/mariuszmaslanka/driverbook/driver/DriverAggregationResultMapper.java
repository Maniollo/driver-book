...
import mariuszmaslanka.driverbook.elastic.ElasticsearchReadException;
import mariuszmaslanka.driverbook.model.DriverInfo;
import mariuszmaslanka.driverbook.model.ShortDriverInfo;
import mariuszmaslanka.driverbook.model.DriversData;
import mariuszmaslanka.driverbook.model.ShortDriverInfoWithTotal;
import mariuszmaslanka.driverbook.model.NationalityInfo;
...
class DriverAggregationResultMapper {

  private static final TypeReference<DriverInfo> DRIVER_DATA_TYPE_REFERENCE = new TypeReference<>() {
  };
  private static final TypeReference<ShortDriverInfo> DRIVER_INFO_TYPE_REFERENCE = new TypeReference<>() {
  };
  private final ObjectMapper objectMapper;

  DriversData map(SearchResponse searchResponse) {
    Aggregations aggregations = searchResponse.getAggregations();
    return DriversData.builder()
        .drivers(getDriversList(searchResponse))
  ...
  private List<DriverInfo> getDriversList(SearchResponse searchResponse) {
    return stream(searchResponse.getHits().getHits())
        .map(hit -> toOutputObject(hit, DRIVER_DATA_TYPE_REFERENCE))
        .sorted(Comparator.comparing(DriverInfo::getDriverId))
        .collect(toList());
  }


  private Map<String, NationalityInfo> getByNationalityResults(Terms aggregation) {
    return aggregation.getBuckets().stream()
        .collect(toMap(MultiBucketsAggregation.Bucket::getKeyAsString, this::mapToNationalityResult, (a, b) -> b, TreeMap::new));
  }
  ...
  private Map<String, ShortDriverInfoWithTotal> getByTitlesResults(Terms aggregation) {
    return aggregation.getBuckets().stream()
        .collect(toMap(MultiBucketsAggregation.Bucket::getKeyAsString, this::mapToTitlesResult, (a, b) -> b, TreeMap::new));
  }
  ...
  private Map<String, ShortDriverInfoWithTotal> getByActiveResults(Terms aggregation) {
    return aggregation.getBuckets().stream()
        .collect(toMap(this::mapKey, this::mapToTitlesResult, (a, b) -> b, TreeMap::new));
  }
